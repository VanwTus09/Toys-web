...
const Promotion = ({ title, price, date, onClick }) => {
  ...
  return (
    <div className="flex flex-1 p-2 w-fit min-w-fit bg-stone-500 text-white">
      <div className="flex flex-nowwrap gap-3 justify-start">
        <div className="border p-2 ">
          ...
          <div className=" flex items-end xs:gap-44 gap-38 ">
            <button
              onClick={onClick}
              className="text-white bg-gray-400 p-1 whitespace-nowrap"
            >
              Xem chi tiết
            </button>
            <p className="text-xs ml-10 p-2 whitespace-nowrap">
              *Áp dụng với 1 số sản phẩm
            </p>
          </div>
        </div>
      </div>
